...
namespace Scrum.Model
{
    public class TarefaMap : ClassMap<Tarefa>
    {
        public TarefaMap()
        {
            ...
            Map(x => x.Data).Not.Nullable().Default("GETDATE()");
            Map(x => x.Hora).Not.Nullable().CustomType("TimeAsTimeSpan").Default("GETDATE()");
            Map(x => x.Descricao).Not.Nullable();
            Map(x => x.Pontuacao).Not.Nullable();
            Map(x => x.Status).Not.Nullable().Check("Status IN ('A Fazer', 'Fazendo', 'Feito')");
